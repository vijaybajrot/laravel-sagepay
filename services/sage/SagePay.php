...
class SagePay
{
    ...
    {
        $formPassword = $this->api->config->getFormPassword();
        $decrypt = SagepayUtil::decryptAes($cryptQS, $formPassword[$this->api->config->env]);
        $decryptArr = SagepayUtil::queryStringToArray($decrypt);
        return $string === true  ? $decrypt : $decryptArr ;
    }
    ...
    public function addBasketItems($items)
    {
        if(is_array($items)){
            foreach ($items as $itemKey => $item) {
                if(is_array($item)){
                    return $this->_addItemInBasket($item);
                }
            }
